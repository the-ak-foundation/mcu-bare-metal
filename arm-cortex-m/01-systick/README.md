...
## What Changed From 00-Minimal-C

### Vector Table

`00-minimal-c` only needs two entries:
...
`00-minimal-c` uses a busy-loop delay. This example uses SysTick, so timing no longer depends on the number of `nop` instructions or compiler optimization.
